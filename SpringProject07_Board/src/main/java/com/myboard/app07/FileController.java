...
public class FileController {
	...
	public String fileInsert(FileBoardDTO fboard, HttpSession session) {
		String uploadFolder = session.getServletContext().getRealPath("/")+"\\resources\\img"; // 경로 불러오기
		System.out.println("uploadFolder  : " + uploadFolder);
		UUID uuid = UUID.randomUUID();
		MultipartFile f = fboard.getUpload();
		String uploadFileName="";
		if(!f.isEmpty()) { // 파일 업로드 했다면
	...
	public String fileList(Model model) {
			List<FileBoardDTO> flist = bservice.fileList();
	...
	public String upload(MultipartFile[] uploads, Model model) {
		String uploadFolder = "C:\\sts_workspace\\SpringProject07_Board\\src\\main\\webapp\\resources";
		...
		for(MultipartFile multipartFile : uploads) {
			...
			// model.addAttribute("uploadImage", uploadFileName);
		}
		return "fileResult";
	}
}
